...
const botApiCompatibilityService_1 = __importDefault(require("./botApiCompatibilityService"));
...
class InternalBotRuntimeService {
    async deliverToDirectWebhook(bot, event, payload) {
        if (!bot.webhookUrl) {
            return;
        }
        let lastError;
        for (let attempt = 1; attempt <= DIRECT_WEBHOOK_MAX_ATTEMPTS; attempt += 1) {
            try {
                await axios_1.default.post(bot.webhookUrl, payload, {
                    headers: {
    ...
    async sendDefaultCommandReply(bot, message) {
        ...
        const reply = await (0, botService_1.sendBotMessage)(bot, {
            chatId: message.chatId,
            content: helpText,
            type: 'TEXT',
        });
        index_1.io.to(`chat:${message.chatId}`).emit('message:new', reply);
    }
    async sendUnavailableCommandReply(bot, message, command) {
        const reply = await (0, botService_1.sendBotMessage)(bot, {
            chatId: message.chatId,
    ...
    async hasExternalRuntime(botId) {
        const [webhookCount, bot] = await Promise.all([
            prisma_1.default.webhook.count({
                where: {
                    botId,
                    isActive: true,
                },
    ...
    async dispatchToBot(botId, event, data) {
        ...
        await Promise.allSettled([
            (0, webhookService_1.deliverToBotWebhooks)(botId, event, payload),
            this.deliverToDirectWebhook(bot, event, payload),
        ]);
    }
    async dispatchChatMessageEvent(messageId, event) {
        ...
        const installations = await prisma_1.default.botChatInstallation.findMany({
            where: {
        ...
        await Promise.allSettled(bots.map(async (bot) => {
            ...
            if (event === 'message.created') {
                await botApiCompatibilityService_1.default.publishMessageUpdate(bot.id, message.id, 'message');
            }
            else if (event === 'message.updated') {
                await botApiCompatibilityService_1.default.publishMessageUpdate(bot.id, message.id, 'edited_message');
            }
...
exports.default = new InternalBotRuntimeService();
//# sourceMappingURL=internalBotRuntimeService.js.map