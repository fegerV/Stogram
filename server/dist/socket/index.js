...
const scheduleMissedCallTimeout = (io, callId, chatId) => {
    clearCallTimeout(callId);
    callTimeouts.set(callId, setTimeout(async () => {
        try {
            const currentCall = await prisma_1.default.call.findUnique({
                where: { id: callId },
            });
...
const scheduleGroupCallTimeout = (io, callId, chatId) => {
    clearCallTimeout(callId);
    callTimeouts.set(callId, setTimeout(async () => {
        try {
            const currentCall = await prisma_1.default.call.findUnique({
...
const ensureChatMembership = async (chatId, userId) => prisma_1.default.chatMember.findFirst({
    where: { chatId, userId },
    select: { chatId: true },
});
const getAccessibleMessage = async (messageId, userId) => prisma_1.default.message.findFirst({
    where: {
        id: messageId,
        chat: {
            members: {
...
const initSocketHandlers = (io) => {
    io.use(async (socket, next) => {
        try {
            ...
            const decoded = jsonwebtoken_1.default.verify(token, (0, authConfig_1.getJwtSecret)());
    ...
    io.on('connection', async (socket) => {
            ...
            try {
                ...
                for (const member of chatMembers) {
                    if (member.userId !== userId &&
                        ...
                        await telegramService_1.default.sendNotification(member.user.telegramId, `Новое сообщение от *${senderName}*\n\n${content}`);
                    }
                }
                const bridges = await prisma_1.default.telegramChatBridge.findMany({
                    where: {
                ...
                for (const bridge of bridges) {
                    try {
                        await telegramService_1.default.syncMessageToTelegram(bridge.id, message);
                    }
                    catch (error) {
        ...
        socket.on('call:initiate', async (data) => {
            try {
                ...
                const existingCall = await prisma_1.default.call.findFirst({
                    where: {
                        chatId,
                ...
                await prisma_1.default.callParticipant.create({
                    data: {
                        callId: call.id,
                        userId,
                    },
        ...
        socket.on('call:answer', async ({ callId }) => {
            try {
                ...
                await prisma_1.default.callParticipant.upsert({
                    where: {
                        callId_userId: {
                            callId,
                            userId,
        ...
        socket.on('call:group:join', async ({ callId }) => {
            try {
                ...
                const updatedCall = await prisma_1.default.call.findUnique({
                    where: { id: callId },
                    include: {
                        participants: {
                            where: { leftAt: null },
        ...
        socket.on('call:group:leave', async ({ callId }) => {
            try {
                ...
                const remainingParticipants = await prisma_1.default.callParticipant.findMany({
                    where: {
                        callId,
                        leftAt: null,
                    },
        ...
        socket.on('call:group:invite', async ({ callId, targetUserId }) => {
            try {
                ...
                // Get initiator info
                const initiator = await prisma_1.default.user.findUnique({
