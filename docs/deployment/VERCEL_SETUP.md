...
**ВАЖНО**: Переменные окружения настраиваются в Vercel Dashboard, а не в `vercel.json`. Vercel автоматически передает их в процесс сборки.
...
**Примечание**: 
- Переменные с префиксом `VITE_` автоматически доступны в коде через `import.meta.env.VITE_API_URL`
- После добавления переменных нужно пересобрать проект (redeploy)
- Можно настроить разные значения для Production, Preview и Development окружений
