...
5. Настройте проект:
   ...
   - **Build Command**: `npm run build` ⚠️ **НЕ используйте `cd client && npm run build`**
   - **Output Directory**: `dist` (относительно client/)
   - **Install Command**: `npm install` ⚠️ **НЕ используйте `cd client && npm install` или `cd client && npm ci`**

**⚠️ КРИТИЧЕСКИ ВАЖНО**: 
- Если Root Directory = `client`, то команды УЖЕ выполняются в папке `client/`
- **НЕ добавляйте** `cd client` в команды - это вызовет ошибку!
- Используйте просто `npm install` и `npm run build` без `cd client`
...
### Проблема: Error "Command 'cd client && npm ci' exited with 1"

**Причина**: В настройках Vercel Dashboard указана команда с `cd client`, но Root Directory уже установлен на `client`.

**Решение**:
1. Откройте Vercel Dashboard > Settings > General
2. Найдите раздел "Build & Development Settings"
3. Проверьте **Install Command**:
   - ❌ **НЕПРАВИЛЬНО**: `cd client && npm ci` или `cd client && npm install`
   - ✅ **ПРАВИЛЬНО**: `npm install` или `npm ci`
4. Проверьте **Build Command**:
   - ❌ **НЕПРАВИЛЬНО**: `cd client && npm run build`
   - ✅ **ПРАВИЛЬНО**: `npm run build`
5. Убедитесь, что **Root Directory** = `client`
6. Сохраните изменения и запустите новый деплой

**Почему это происходит**: 
- Если Root Directory = `client`, то все команды УЖЕ выполняются в папке `client/`
- Добавление `cd client` пытается перейти в `client/client/`, что не существует
- Это вызывает ошибку "exited with 1"

### Проблема: Build fails с ошибкой "Cannot find module"

**Решение**: 
1. Убедитесь, что Root Directory = `client`
2. Проверьте, что Install Command = `npm install` (без `cd client`)
3. Убедитесь, что `package.json` существует в папке `client/`

### Проблема: API запросы идут на localhost

**Решение**: 
1. Проверьте, что переменные окружения `VITE_API_URL` и `VITE_WS_URL` установлены в Vercel Dashboard
...
3. Проверьте, что домен указан правильно (с https://)
